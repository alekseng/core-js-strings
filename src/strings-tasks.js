...
function countVowels(str) {
  return str.replace(/[qwrtplkjhgfdszxcvbnm,1-9,0]/gi, '').length;
}
...
function isPalindrome(str) {
  return (
    str.toLocaleLowerCase().replaceAll(' ', '').replace(/[!?,]/gi, '') ===
    str
      .split('')
      .reverse()
      .join('')
...
function findLongestWord(sentence) {
  ...
  let findWord = '';
  for (let i = 0; i < arr.length; i += 1) {
    if (findWord.length < arr[i].length) {
      findWord = arr[i];
    }
  }
  return findWord;
...
function reverseWords(str) {
  const arr = str.split(' ');
  const res = arr.map((el) => el.split('').reverse().join(''));
...
function encodeToRot13(str) {
  ...
  const arr = str.split(' ');
  const res = arr
    .map((el) => el.replace(/[A-Za-z]/g, (el1) => rot13[alphabet.indexOf(el1)]))
    .join(' ');
  return res;
}
